            ...
            <Reveal key={item.period} delay={i * 70}>
              <div className="group grid gap-4 py-8 sm:grid-cols-12 sm:gap-8" style={{ borderTop: "1px solid var(--line)" }}>
                <div className="sm:col-span-3">
                  <span className="display block text-5xl leading-none sm:text-6xl force-ltr" style={{ color: "var(--fg-2)" }}>
                    {item.period}
                  </span>
                </div>
                <div className="sm:col-span-9 sm:ps-8" style={{ borderInlineStart: "1px solid var(--line)" }}>
                  <div className="flex items-center gap-3">
                    <span className="h-2.5 w-2.5 rounded-full" style={{ background: "var(--accent)", boxShadow: "0 0 12px var(--glow)" }} />
                    <h3 className="font-display text-2xl">{pick(item.title, lang)}</h3>
                  </div>
